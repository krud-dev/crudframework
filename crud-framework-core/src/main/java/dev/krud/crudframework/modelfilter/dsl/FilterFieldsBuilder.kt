...
class FilterFieldsBuilder<RootType : PersistentEntity>(private val filterFields: MutableList<FilterField> = mutableListOf(), private val fieldPrefix: String = "") {
    ...
    infix fun KProperty1<RootType, String?>.Equal(target: String) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.Equal, FilterFieldDataType.String, target
        )
    }

    infix fun KProperty1<RootType, Int?>.Equal(target: Int) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.Equal, FilterFieldDataType.Integer, target
        )
    }

    infix fun KProperty1<RootType, Long?>.Equal(target: Long) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.Equal, FilterFieldDataType.Long, target
        )
    }

    infix fun KProperty1<RootType, Double?>.Equal(target: Double) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.Equal, FilterFieldDataType.Double, target
        )
    }

    infix fun KProperty1<RootType, Boolean?>.Equal(target: Boolean) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.Equal, FilterFieldDataType.Boolean, target
        )
    }

    infix fun KProperty1<RootType, Date?>.Equal(target: Date) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.Equal, FilterFieldDataType.Date, target
        )
    }

    infix fun <EnumType : Enum<EnumType>> KProperty1<RootType, EnumType?>.Equal(target: EnumType) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.Equal, target.javaClass.canonicalName, target
        )
    }

    infix fun KProperty1<RootType, UUID>.Equal(target: UUID) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.Equal, FilterFieldDataType.UUID, target
        )
    }

    infix fun KProperty1<RootType, String?>.NotEqual(target: String) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotEqual, FilterFieldDataType.String, target
        )
    }

    infix fun KProperty1<RootType, Int?>.NotEqual(target: Int) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotEqual, FilterFieldDataType.Integer, target
        )
    }

    infix fun KProperty1<RootType, Long?>.NotEqual(target: Long) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotEqual, FilterFieldDataType.Long, target
        )
    }

    infix fun KProperty1<RootType, Double?>.NotEqual(target: Double) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotEqual, FilterFieldDataType.Double, target
        )
    }

    infix fun KProperty1<RootType, Boolean?>.NotEqual(target: Boolean) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotEqual, FilterFieldDataType.Boolean, target
        )
    }

    infix fun KProperty1<RootType, Date?>.NotEqual(target: Date) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotEqual, FilterFieldDataType.Date, target
        )
    }

    infix fun <EnumType : Enum<EnumType>> KProperty1<RootType, EnumType?>.NotEqual(target: EnumType) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotEqual, target.javaClass.canonicalName, target
        )
    }

    infix fun KProperty1<RootType, UUID>.NotEqual(target: UUID) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotEqual, FilterFieldDataType.UUID, target
        )
    }

    infix fun KProperty1<RootType, String?>.Contains(target: String) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.Contains, FilterFieldDataType.String, target
        )
    }

    infix fun KProperty1<RootType, Int?>.GreaterThan(target: Int) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.GreaterThan, FilterFieldDataType.Integer, target
        )
    }

    infix fun KProperty1<RootType, Long?>.GreaterThan(target: Long) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.GreaterThan, FilterFieldDataType.Long, target
        )
    }

    infix fun KProperty1<RootType, Double?>.GreaterThan(target: Double) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.GreaterThan, FilterFieldDataType.Double, target
        )
    }

    infix fun KProperty1<RootType, Date?>.GreaterThan(target: Date) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.GreaterThan, FilterFieldDataType.Date, target
        )
    }

    infix fun KProperty1<RootType, Int?>.GreaterOrEqual(target: Int) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.GreaterEqual, FilterFieldDataType.Integer, target
        )
    }

    infix fun KProperty1<RootType, Long?>.GreaterOrEqual(target: Long) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.GreaterEqual, FilterFieldDataType.Long, target
        )
    }

    infix fun KProperty1<RootType, Double?>.GreaterOrEqual(target: Double) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.GreaterEqual, FilterFieldDataType.Double, target
        )
    }

    infix fun KProperty1<RootType, Date?>.GreaterOrEqual(target: Date) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.GreaterEqual, FilterFieldDataType.Date, target
        )
    }

    infix fun KProperty1<RootType, Int?>.LowerThan(target: Int) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.LowerThan, FilterFieldDataType.Integer, target
        )
    }

    infix fun KProperty1<RootType, Long?>.LowerThan(target: Long) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.LowerThan, FilterFieldDataType.Long, target
        )
    }

    infix fun KProperty1<RootType, Double?>.LowerThan(target: Double) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.LowerThan, FilterFieldDataType.Double, target
        )
    }

    infix fun KProperty1<RootType, Date?>.LowerThan(target: Date) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.LowerThan, FilterFieldDataType.Date, target
        )
    }

    infix fun KProperty1<RootType, Int?>.LowerOrEqual(target: Int) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.LowerEqual, FilterFieldDataType.Integer, target
        )
    }

    infix fun KProperty1<RootType, Long?>.LowerOrEqual(target: Long) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.LowerEqual, FilterFieldDataType.Long, target
        )
    }

    infix fun KProperty1<RootType, Double?>.LowerOrEqual(target: Double) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.LowerEqual, FilterFieldDataType.Double, target
        )
    }

    infix fun KProperty1<RootType, Date?>.LowerOrEqual(target: Date) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.LowerEqual, FilterFieldDataType.Date, target
        )
    }
    ...
    infix fun KProperty1<RootType, String?>.In(target: Collection<String>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.In, FilterFieldDataType.String, *target.toTypedArray()
        )
    }
    ...
    infix fun KProperty1<RootType, Int?>.In(target: Collection<Int>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.In, FilterFieldDataType.Integer, *target.toTypedArray()
        )
    }
    ...
    infix fun KProperty1<RootType, Long?>.In(target: Collection<Long>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.In, FilterFieldDataType.Long, *target.toTypedArray()
        )
    }
    ...
    infix fun KProperty1<RootType, Double?>.In(target: Collection<Double>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.In, FilterFieldDataType.Double, *target.toTypedArray()
        )
    }
    ...
    infix fun KProperty1<RootType, Date?>.In(target: Collection<Date>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.In, FilterFieldDataType.Date, *target.toTypedArray()
        )
    }
    ...
    infix fun <EnumType : Enum<EnumType>> KProperty1<RootType, EnumType?>.In(target: Collection<EnumType>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.In, target.first()::class.java.canonicalName, *(target as Collection<*>).toTypedArray()
        )
    }
    ...
    infix fun KProperty1<RootType, UUID?>.In(target: Collection<UUID>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.In, FilterFieldDataType.UUID, *target.toTypedArray()
        )
    }
    ...
    infix fun KProperty1<RootType, String?>.NotIn(target: Collection<String>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotIn, FilterFieldDataType.String, *target.toTypedArray()
        )
    }
    ...
    infix fun KProperty1<RootType, Int?>.NotIn(target: Collection<Int>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotIn, FilterFieldDataType.Integer, *target.toTypedArray()
        )
    }
    ...
    infix fun KProperty1<RootType, Long?>.NotIn(target: Collection<Long>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotIn, FilterFieldDataType.Long, *target.toTypedArray()
        )
    }
    ...
    infix fun KProperty1<RootType, Double?>.NotIn(target: Collection<Double>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotIn, FilterFieldDataType.Double, *target.toTypedArray()
        )
    }
    ...
    infix fun KProperty1<RootType, Date?>.NotIn(target: Collection<Date>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotIn, FilterFieldDataType.Date, *target.toTypedArray()
        )
    }
    ...
    infix fun <EnumType : Enum<EnumType>> KProperty1<RootType, EnumType?>.NotIn(target: Collection<EnumType>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotIn, target.first()::class.java.canonicalName, *(target as Collection<*>).toTypedArray()
        )
    }
    ...
    infix fun KProperty1<RootType, UUID?>.NotIn(target: Collection<UUID>) {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.NotIn, FilterFieldDataType.UUID, *target.toTypedArray()
        )
    }
    ...
    infix fun KProperty1<RootType, *>.isNull(condition: Boolean) {
        when (condition) {
            true -> filterFields += FilterField(
                effectiveName, FilterFieldOperation.IsNull, FilterFieldDataType.Object, null, null
            )

            false -> filterFields += FilterField(
                effectiveName, FilterFieldOperation.IsNotNull, FilterFieldDataType.Object, null, null
            )
        }
    }
    ...
    fun KProperty1<RootType, Collection<*>>.isEmpty() {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.IsEmpty, FilterFieldDataType.Object, null, null
        )
    }

    fun KProperty1<RootType, Collection<*>>.isNotEmpty() {
        filterFields += FilterField(
            effectiveName, FilterFieldOperation.IsNotEmpty, FilterFieldDataType.Object, null, null
        )
    }
